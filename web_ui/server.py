...
def build_case_review(result):
    ...
    severity_indicators = grading.get("severity_indicators", [])
    lesion_counts = grading.get("lesion_counts", {})
    ...
    if label == "Ungradable":
        review_note = "Image quality is insufficient for reliable screening."
        action = "Please retake the image with a proper retinal camera."
        ...
        action = "Discuss this result with an eye-care professional."
    else:
        review_note = "This is a screening estimate and still requires clinician confirmation."
        action = "Share this result with your eye doctor for confirmation and next steps."
    ...
    quality_status = "PASS" if quality.passed else "NEEDS RETREAT"
    quality_details = []
    if hasattr(quality, 'feedback'):
        quality_details = [f for f in quality.feedback if f != "Pass"]
    quality_text = ", ".join(quality_details) if quality_details else "All quality checks passed"
    
    lines = [
        "=" * 44,
        "      RETINACARE SCREENING REPORT",
        "=" * 44,
        "",
        f"SCREENING RESULT: {finding_names.get(label, label)}",
        f"CONFIDENCE: {confidence:.0%} ({review_note})",
        "",
        "-" * 44,
        "WHAT WAS FOUND:",
        "-" * 44,
    ...
    lines.extend([
        "LESION COUNTS:",
        f"  * Microaneurysms: {lesion_counts.get('microaneurysms', 0)}",
        f"  * Exudates: {lesion_counts.get('exudates', 0)}",
        f"  * Hemorrhages: {lesion_counts.get('haemorrhages', 0)}",
        f"  * Neovascularization: {lesion_counts.get('neovascularisation', 0)}",
        "",
    ])
    ...
    if probabilities and len(probabilities) == 5:
        ...
        for i, (lbl, prob) in enumerate(zip(labels, probabilities)):
            bar = "#" * int(prob * 20)
            marker = " <-- SELECTED" if i == grading.get("level", -1) else ""
            lines.append(f"  {lbl}: {prob:.1%} {bar}{marker}")
        lines.append("")
    
    lines.extend([
        "-" * 44,
        f"IMAGE QUALITY: {quality_status}",
        f"  {quality_text}",
        "",
        f"MODEL: {method}",
        "",
        ...
        "  * A clinician MUST confirm every result",
        "  * Blood tests (HbA1c, glucose) are needed to confirm diabetes",
        f"  * {action}",
        "",
        "URGENT CARE: Seek immediate eye care for:",
        "   Sudden vision loss * Curtain/shadow in vision",
        "   Many new floaters * Flashes of light",
        "   Severe eye pain * Rapidly worsening vision",
        "",
        "=" * 44,
        "RetinaCare v1.0 | For clinical discussion only",
        "=" * 44,
    ])
...
def build_result_details(result):
    ...
    return {
        ...
        "lesion_counts": grading.get("lesion_counts", {}),
        "probabilities": grading.get("probabilities", []),
        "label": grading.get("label", "Ungradable"),
    }
...
def run_model():
    ...
    force_rerun = request.form.get('force', '').lower() in ('1', 'true', 'yes')
